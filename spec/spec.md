...
## 13. Phase 8 Features (Polish & Tính năng nâng cao)
> Kế hoạch triển khai chi tiết: xem `plan/plan.md`

### Phase A — Dark Mode + Toast Notifications
1. [x] **Dark Mode Toggle:** Hoàn thiện ThemeToggle component đã build vào thanh điều hướng (Sidebar desktop + Mobile header). ThemeContext + CSS variables đã sẵn sàng, chỉ cần wire nút vào UI.
2. [x] **Toast Notifications:** Thay thế toàn bộ `alert()` bằng toast thông báo đẹp (ToastContext đã build sẵn). Áp dụng cho Transactions, Accounts, More.

### Phase B — Sửa giao dịch
3. [x] **Sửa giao dịch (Edit Transaction):** Cho phép chỉnh sửa số tiền, danh mục, tài khoản, ghi chú của giao dịch đã nhập. Tái sử dụng form hiện có với mode `edit`. Tự động cập nhật số dư tài khoản khi thay đổi.

### Phase C — Tìm kiếm & Lọc nâng cao
4. [x] **Tìm kiếm & Lọc Giao dịch:** Thêm thanh tìm kiếm theo từ khóa (ghi chú/danh mục), lọc theo khoảng ngày (Từ ngày – Đến ngày), lọc theo danh mục. Các bộ lọc hoạt động kết hợp với nhau.

### Phase D — Chuyển tiền
5. [x] **Chuyển tiền giữa tài khoản (Transfer):** Loại giao dịch mới bên cạnh Thu/Chi. Chọn tài khoản nguồn và tài khoản đích, tự động cập nhật số dư cả hai. Hiển thị riêng biệt trong danh sách với icon mũi tên 2 chiều.

### Phase E — Quản lý Ngân sách
6. [x] **Quản lý Ngân sách (Budget):** Đặt hạn mức chi tiêu theo danh mục mỗi tháng. Hiển thị progress bar trên trang Báo cáo: xanh (<70%), vàng (70–90%), đỏ (>90% hạn mức).
